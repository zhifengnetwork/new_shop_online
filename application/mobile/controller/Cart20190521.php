    ...
    public function cart3(){
        ...
        try {
            ...
            if ($_REQUEST['act'] == 'submit_order') {
                ...
                $worder = Db::name('order')->field('order_id,pay_status,user_id,order_amount,total_amount')->where('order_sn', $order['order_sn'])->find();
                if($worder['pay_status'] == 1){
                    $userinfo = Db::name('users')->field('user_id,nickname,openid,mobile')->where('user_id',$worder['user_id'])->find();
                    if($userinfo && $userinfo['openid']){
                        $goods_name = Db::name('order_goods')->where('order_id', $order['order_id'])->value('goods_name');
                        $wmoney = $worder['order_amount'] > 0 ? $worder['order_amount'] : $worder['total_amount'];
                        $this->Purchase_Success($userinfo['openid'],'商品支付成功！',$goods_name,'支付成功！',$wmoney ,'欢迎再次购买！');
                    }
                }
