...
class Sales extends Model
{
	...
	public function reward($parents_id,$user_level,$is_repeat)
	{
		...
		if ($user_level > 0) {
			...
			if ($my_prize > 0) {
                ...
                //微信公众号发消息
                $this->distribution_success($user['openid'],'你好，你已分销商品成功。',$order['goods_name'],$goods_price,$my_prize,time(),'感谢你的使用。');
			}
		}
			...
			if ($user_level == $value['distribut_level']) {
				...
				if (!$is_prize) {
					...
					if ($money > 0) {
                        ...
                        //微信公众号发消息
                        $this->distribution_success($value['openid'],'你好，你已分销商品成功。',$order['goods_name'],$goods_price,$money,time(),'感谢你的使用。');
					}
				}
				$msg = "同级奖 ";
				$distribut_type = 4;
	...
	//分销模板消息
    public function distribution_success($openid,$title,$goods,$goods_price,$money,$time,$remark,$url=''){
	...
	{
		$all = M('users')->where('user_id','in',$parents_id)->column('openid,user_id,first_leader,distribut_level,is_lock,user_money,distribut_money,is_distribut');
		$result = array();
